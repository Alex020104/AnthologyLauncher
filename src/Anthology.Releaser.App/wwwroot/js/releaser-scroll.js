(() => {
    const contentSelector = "#releaser-content-scroll";
    const trackSelector = ".releaser-scroll-track";
    const thumbSelector = ".releaser-scroll-thumb";

    function bindScrollControl() {
        const content = document.querySelector(contentSelector);
        const track = document.querySelector(trackSelector);
        const thumb = track?.querySelector(thumbSelector);
        const control = track?.closest(".releaser-scroll-control");
        ...
        track.addEventListener("pointerdown", event => {
            if (event.target !== thumb) {
                scrollToPointer(event.clientY, metrics().thumbHeight / 2);
            }
        });

        control.querySelector('[data-scroll-direction="up"]')?.addEventListener("click", () => {
            content.scrollBy({ top: -Math.max(140, content.clientHeight * .7), behavior: "smooth" });
        });
