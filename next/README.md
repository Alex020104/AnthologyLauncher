...
Это первая вертикальная версия архитектуры, а не готовый публичный релиз. Учётные записи, постоянная база сообщества, админ-панель, распаковка пакетов и финальный production deployment ещё находятся в следующих этапах.
...
$env:ANTHOLOGY_GAME_ROOT = "X:\путь\к\Anomaly-1.5.3-Anthology 2.1"
...
Разработка ведётся в ветке `feature/anthology-launcher-next` и каталоге `next/`. Старый `AnomalyLauncher.exe`, Python-код, текущие manifests/releases и ветка `main` не заменяются. Подключение нового updater-канала будет отдельным явно контролируемым этапом после тестирования.
