...
- **Выпустить аддон** copies only the selected archive, recalculates size and SHA-256, refreshes the signed catalog and uploads the changed files without rebuilding game/MO2 archives.
- **Снять аддон** removes its archive and catalog entry from publication while retaining the editable card as a draft.
