...
Кнопка «Полный MO2» открывает настоящее окно `ModOrganizer.exe`, поэтому
FOMOD-установщики, загрузки, анализ конфликтов, плагины, INI, профили и прочие
штатные возможности не заменяются урезанной реализацией. Нативное переподчинение
Qt-окна через Win32 отключено: в используемой сборке MO2 2.5.0 оно вызывало
`0xc0000005`. Лаунчер синхронизирует выбранный профиль с `ModOrganizer.ini`, а
прямой запуск сборки выполняет `run -e <executable>` без проблемного `-p`.
