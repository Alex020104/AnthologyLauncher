...
Кнопка «Полный MO2» встраивает настоящее окно `ModOrganizer.exe` в рабочую область
лаунчера. Поэтому FOMOD-установщики, загрузки, анализ конфликтов, плагины, INI,
профили и прочие штатные возможности не заменяются урезанной реализацией. При
закрытии Anthology Next окно MO2 безопасно отсоединяется и не завершается принудительно.
Прямой запуск выбранной сборки использует официальный CLI MO2:
`-p <profile> run -e <executable>`.
