...
Новости и «Информация» являются обычными записями рабочего проекта. Их разрешено создавать, редактировать, менять местами, снимать с публикации и полностью удалять. Новость поддерживает собственную дату, текстовые блоки, картинки, URL и видео. Информационная вкладка дополнительно поддерживает карточки-подразделы — так реализован полностью редактируемый каталог сюжетов. При первом переходе со старой схемы прежние стартовые тексты импортируются как черновики ровно один раз.
